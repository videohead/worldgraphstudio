...
| Story import and export | The default-enabled WordPress feature plugin exposes administrator-only canonical validation/import, synchronous decomposition compatibility, resumable decomposition-job status/step/cancel, and Project JSON/Markdown export routes. Its Import screen uses JavaScript to advance a server-owned structural, two-pass decomposition job one checkpoint at a time, resume after an interrupted request, cancel without deleting the source, and enter the existing preview/confirm workflow only after final version 1.2 validation. An optional disabled-by-default nested plugin can replace lexical evidence matching with private, transient WPVDB vectors while exposing only sanitized backend/count status. The Next.js app has no creator adapter, browser-user authorization, upload/retention controls, job UI, preview/confirm UI, or export download flow | Partial — authentication-blocked | Authenticate the browser user, map equivalent WordPress and object capabilities, define typed DTOs for persisted attachments, immutable job identity, safe structural progress, step/retry/resume/cancel, preview/confirmation, and all three export formats; invoke shared PHP retrieval behavior rather than adding browser-side embeddings; never expose source chunks, evidence, vectors, prompts, graph memory, reasoning, or credentials; expose retained-source deletion guidance, implement accessible long-running/error/cancel/success states, and add cross-stack tests |
...
persisted-upload, immutable decomposition-job identity, safe structural
progress, retry/resume/cancel states, explicit confirmation, source retention,
or download outcomes. The authoritative chunk plan and two-pass graph assembly
must remain shared PHP behavior rather than being reimplemented in Next.js.
Interchange therefore remains explicitly authentication-blocked parity debt.
