...
| Published Story Graph displays | `/story`, `/story/[type]`, and `/story/[type]/[slug]` use a typed public `wp/v2` adapter for Projects, Worlds, Characters, Scenes, Props, and Sounds/Songs. Specialized cards and details consume SCF plus the read-only `worldgraph_display` projection for media, ordered published Shots, and Project analysis | Partial | Add adapter contract fixtures, browser accessibility/interaction coverage, all Story-entity cache lifecycle tests, and an agreed public preview scope |
...
| Story Graph workspace | Public Story collection/detail routes browse published content, but deliberately provide no draft, create, edit, or administrative UI. WordPress now adds explicit Project publish guidance that lists exactly which required Project fields are still missing before publish can succeed. | Missing | Add a permission-aware creator contract and browse/edit workflows; the anonymous `wp/v2` projection must not be treated as workspace authorization |
...
| Asset generation | WordPress now exposes Template-conditional sanitized `run_controls`, validates optional direct/batch image, video, and audio values, and provides a Project-only demonstration plan with dependency-aware generation, cancellation, and rough-cut assembly status. Headless has no authenticated plan, generate, run-control, batch progress, cancel, history, assembly, or provenance path | Missing — authentication-blocked | First deliver browser-user authentication and object-level authorization, then define creator adapter/DTOs for `run_controls`, `run_values`, `image_run_values`, `video_run_values`, `audio_run_values`, demonstration tasks, and assembly progress/results; invoke the shared PHP validation/freeze/idempotency behavior; cover complete job and assembly states, accessibility, and cross-stack tests |
