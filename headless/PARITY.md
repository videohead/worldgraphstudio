...
| Story intelligence | WordPress and published headless Project details now render the same deterministic Development Compass from the shared graph analyzer, including a next-change question when coverage checks are clear. WordPress adds normal edit/create navigation; headless uses the privacy-filtered public projection. Search, continuity, summary, dramaturgy, authenticated creator actions, and interactive analytics remain absent in headless | Partial | Add runtime adapter fixtures and browser coverage for the Compass, then deliver equivalent authorized outcomes for the remaining shipped workflows or record explicit exceptions |
