...
| Published Story Graph displays | WordPress CPT archives and singles now include a reusable Story Graph directory linking to all six public CPT collections; Project details also link to the aggregate Storyboard view. `/story`, `/story/[type]`, and `/story/[type]/[slug]` provide the equivalent headless collection and detail entry points through a typed public `wp/v2` adapter. Specialized cards and details consume SCF plus the read-only `worldgraph_display` projection for media, ordered published Shots, Project analysis, and the deterministic Development Compass after private nodes are filtered | Partial | Add adapter contract fixtures, browser accessibility coverage, all Story-entity cache lifecycle tests, and an agreed public preview scope |
| Pages, taxonomies, authors, media, public search, and preview | The fixed `/about` route mirrors the theme-owned About page, and data helpers exist for several WordPress resources; arbitrary public Pages, taxonomies, authors, media, public search, and Draft Mode still have no corresponding user routes | Missing | Deliver the agreed public routes and authenticated preview, or record a narrower approved scope |
...
| Asset generation | WordPress now exposes Template-conditional sanitized `run_controls`, an effective bounded prompt preview/policy, and explicit Connection+Template-keyed defaults with Template → Project → item → one-run precedence. It validates direct/batch image, video, and audio values, provides a Project-only demonstration plan with dependency-aware generation, cancellation, and rough-cut assembly status, and can execute the reviewed Seedance 2.5 text-to-video and image-to-video Templates through CyberBara. Headless has no authenticated plan, prompt preview, defaults editor, provider-aware generate path, run-control, batch progress, cancel, history, assembly, or provenance path | Missing — authentication-blocked | First deliver browser-user authentication and object-level authorization, then define creator adapter/DTOs for prompt preview, `run_controls`, `run_defaults`, `run_values`, `image_run_values`, `video_run_values`, `audio_run_values`, Seedance reference-image selection, demonstration tasks, and assembly progress/results; invoke the shared PHP validation/freeze/idempotency behavior; cover complete job and assembly states, accessibility, and cross-stack tests |
| Connections and Templates | `/connections` provides part of the ComfyUI catalog workflow through a server-side v1 adapter, but it filters the catalog request to ComfyUI. WordPress now also manually configures `seedance_25` and provisions two CyberBara-backed Templates; headless exposes neither that Connection nor those Templates. LiteLLM is available to WordPress as a dedicated OpenAI-compatible base-LLM and story-generation Connection, while headless exposes no Connection administration. The wp-admin Template Workflow Test (prompt-driven test run returning an Asset number, plus a prompt assistant) has no headless route, but it adds no new API contract: it composes the existing `worldgraph/v1/generation` submit/status and `worldgraph/v1/ai/chat` endpoints | Partial — production-blocked | Authenticate and authorize the browser user, define provider-neutral stable DTOs, expose the reviewed Seedance Templates and required image binding, preserve upstream errors, cover generic agreed workflows including the Template test run, and add contract/E2E tests |
| Provider credential and OAuth setup | Higgsfield is configured on a saved wp-admin Connection through an administrator-only external authorization-code + PKCE flow and encrypted token envelope. Seedance 2.5 via CyberBara is also configured manually on a saved wp-admin Connection with a CyberBara API key or environment reference. Headless has no safe browser-user authorization or secret-management control for either path | WP-only | Explicit narrow exception: account authorization, disconnect, and provider credential setup remain WordPress/deployment administration because WordPress owns the Connection record, capability/nonce boundary, callback URI where applicable, and secret-storage boundary. This does not exempt creator generation, job state, Templates, or general Connection visibility from the existing parity debt |
| Story intelligence | WordPress and published headless Project details now render the same deterministic Development Compass from the shared graph analyzer, including a next-change question when coverage checks are clear. WordPress project entity-count rows now link to the current Project dossier or the corresponding public CPT archive; headless uses the privacy-filtered public projection. Search, continuity, summary, dramaturgy, authenticated creator actions, and interactive analytics remain absent in headless. The WordPress-only admin analytics tables link entity and character names to their native edit screens; this is a narrow wp-admin navigation exception, not a headless capability. | Partial | Add runtime adapter fixtures and browser coverage for the Compass, then deliver equivalent authorized outcomes for the remaining shipped workflows or record explicit exceptions |
| Story import and export | The default-enabled WordPress feature plugin exposes administrator-only canonical validation/import, persisted-attachment decomposition-job creation plus status/step/cancel, bounded synchronous decomposition compatibility, and Project JSON/Markdown export routes. Job creation returns HTTP 202, a safe status projection, and its item `Location`; canonical JSON is directed to synchronous validation/preview/import, while non-canonical synchronous decomposition is restricted to at most 1,400 UTF-8 characters and exactly one initial span. Its Import screen uses JavaScript to advance a server-owned structural, two-pass decomposition job one checkpoint at a time, resume after an interrupted request, cancel without deleting the source, and enter the existing preview/confirm workflow only after final version 1.2 validation. An optional disabled-by-default nested plugin can replace lexical evidence matching with private, transient WPVDB vectors while exposing only sanitized backend/count status. The Next.js app has no creator adapter, browser-user authorization, upload/retention controls, job UI, preview/confirm UI, or export download flow | Partial — authentication-blocked | Authenticate the browser user, map equivalent WordPress and object capabilities, define typed DTOs for persisted attachments, collection creation and its 202/409/`Location` semantics, immutable job identity, safe structural progress, step/retry/resume/cancel, preview/confirmation, and all three export formats; invoke shared PHP retrieval behavior rather than adding browser-side embeddings; never expose source chunks, evidence, vectors, prompts, graph memory, reasoning, or credentials; expose retained-source deletion guidance, implement accessible long-running/error/cancel/success states, and add cross-stack tests |
...
also covers manual provider-secret entry such as the CyberBara API key. It does
not make Higgsfield or Seedance generation, or generic Connection/Template
management, a WP-only product capability; those creator-facing outcomes remain
subject to the existing authentication-blocked parity requirements.

The Seedance 2.5 via CyberBara delivery therefore widens the concrete behavior
missing from the existing Asset generation and Connections and Templates rows.
No headless adapter, provider-neutral Connection view, Template selection,
reference-image binding, submission, polling, result display, or cross-stack
test was added with the WordPress adapter. No broader WP-only exception has
been approved. This ledger records that unresolved parity gap; it does not
satisfy or waive the headless deliverable.
...
persisted-upload, resumable-job collection creation, 202 `Location` handling,
the `worldgraph_story_decomposition_not_required` and
`worldgraph_story_decomposition_job_required` 409 branches, immutable
decomposition-job identity, safe structural progress, retry/resume/cancel
states, explicit confirmation,
source retention, or download outcomes. The authoritative chunk plan and
two-pass graph assembly must remain shared PHP behavior rather than being
reimplemented in Next.js. Interchange therefore remains explicitly
