...
| Story RAG Decomposer | Optional private evidence retrieval | Bounded evidence → WPVDB embedding provider → private transient vectors and related evidence | Implements the Story Import & Export retrieval hooks with same-user/same-run cosine ranking; retains up to 128 uniformly sampled numeric vectors in private transients capped by the owning run's fixed deadline and requests terminal cleanup, including separately scoped synchronous runs; never writes private text or vectors to WPVDB's embeddings table; HMAC-scopes and best-effort evicts WPVDB Core's brief shared object-cache entry; every dependency, configuration, provider, or vector failure falls back to lexical retrieval | Bundled and disabled by default; **requires** WPVDB separately installed and activated as the top-level `wp-content/plugins/wpvdb` plugin, plus an active WPVDB embedding provider/model |
...
Story Import & Export and its built-in lexical decomposer do not require
WPVDB. The requirement in the Story RAG Decomposer row applies only when that
separate long-form retrieval enhancement is enabled; World Graph Studio does
not install WPVDB or bundle it inside the nested RAG directory. In the target
WordPress installation, the required sibling path is
`wp-content/plugins/wpvdb`.
...
| Story document decomposition | Persisted story upload → canonical JSON preview → Story Graph | Extracts bounded text from JSON/TXT/Markdown/Fountain/RTF/PDF/EPUB/DOCX/ODT; canonical JSON skips the LLM, while other sources use chapter/section/Scene-aware spans, separate neighbor context, evidence and evolving-graph synthesis passes, resumable checkpoints, and preview/confirm before commit; PDF requires a text layer | WordPress Import screen; `POST worldgraph/v1/import/decompositions` job creation plus item status/step/cancel routes; synchronous `POST worldgraph/v1/import/decompose` only for canonical JSON or non-canonical input of at most 1,400 UTF-8 characters with exactly one initial span; source attachment remains in uploads |
| Optional vector evidence retrieval | Private transient evidence → related evidence bundle | Uses the active WPVDB provider/model for bounded embedding and top-three same-user/same-run cosine neighbors; no manuscript/evidence/query text or vectors enter the WPVDB embeddings table, while HMAC-scoped shared object-cache entries are best-effort evicted before and after each call; failures retain the lexical bundle | Separately install and activate WPVDB at the top-level `wp-content/plugins/wpvdb`, configure its embedding provider/model, then enable **Story RAG Decomposer** under World Graph Studio Plugins; no new REST route |
...
| [Seedance 2.5 via CyberBara](plugins/SEEDANCE.md) | CyberBara REST API | Provisions fixed text-to-video and image-to-video Templates for `seedance-2.5`, uploads authorized reference images, polls asynchronous tasks, validates the full bounded output list, and imports every distinct final video; this is not a direct ByteDance, BytePlus, Volcengine, or Dreamina connection | CyberBara API key |
