...
function worldgraph_child_register_story_blocks() {
	...
	register_block_type(
		...
		array(
			...
			'editor_script'   => 'worldgraph-child-story-blocks',
			'render_callback' => 'worldgraph_child_render_storyboard_block',
		)
	);
}
...
/**
 * Render all published Shots as storyboard panels.
 *
 * @param array<string, mixed> $attributes Block attributes.
 * @return string
 */
function worldgraph_child_render_storyboard_block( $attributes = array() ) {
	unset( $attributes );
	worldgraph_child_enqueue_story_display_script();
	...
	$wrapper_attributes = function_exists( 'get_block_wrapper_attributes' )
		? get_block_wrapper_attributes( array( 'class' => 'wg-storyboard-block' ) )
		: 'class="wp-block-worldgraph-storyboard wg-storyboard-block"';

	ob_start();
	?>
	<div <?php echo $wrapper_attributes; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Generated by core. ?> >
		<section class="wg-storyboard" aria-labelledby="wg-storyboard-title">
			<header class="wg-storyboard__header">
				<p class="wg-story-kicker"><?php esc_html_e( 'Visual plan', 'worldgraph-child' ); ?></p>
				<h1 id="wg-storyboard-title"><?php esc_html_e( 'Storyboard', 'worldgraph-child' ); ?></h1>
				<p><?php echo esc_html( sprintf( _n( '%s published Shot', '%s published Shots', (int) $query->found_posts, 'worldgraph-child' ), number_format_i18n( (int) $query->found_posts ) ) ); ?></p>
			</header>

			<?php if ( $query->have_posts() ) : ?>
				<ol class="wg-storyboard__grid">
					<?php while ( $query->have_posts() ) : $query->the_post(); ?>
						<?php
						$shot       = get_post();
						$media      = function_exists( '\WorldGraph\Utils\worldgraph_get_story_display_media' ) ? \WorldGraph\Utils\worldgraph_get_story_display_media( $shot->ID, false ) : array();
						$figure     = worldgraph_child_story_featured_figure( $media, get_the_title( $shot ), 'wg-storyboard-panel__media' );
						$shot_type  = worldgraph_child_story_field_text( $shot->ID, 'shot_type' );
						$camera     = worldgraph_child_story_field_text( $shot->ID, 'camera_angle' );
						$lens       = worldgraph_child_story_field_text( $shot->ID, 'lens' );
						$duration   = worldgraph_child_story_field_text( $shot->ID, 'duration' );
						$description = worldgraph_child_story_summary( $shot, array( 'shot_description' ) );
						?>
						<li class="wg-storyboard-panel">
							<div class="wg-storyboard-panel__frame">
								<?php echo $figure ? $figure : worldgraph_child_story_empty_state( __( 'Frame pending', 'worldgraph-child' ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Renderer escapes media. ?>
							</div>
							<div class="wg-storyboard-panel__body">
								<p class="wg-storyboard-panel__number"><?php echo esc_html( sprintf( __( 'Shot %s', 'worldgraph-child' ), number_format_i18n( $shot->menu_order ? $shot->menu_order : $query->current_post + 1 ) ) ); ?></p>
								<h2><a href="<?php echo esc_url( get_permalink( $shot ) ); ?>"><?php echo esc_html( get_the_title( $shot ) ); ?></a></h2>
								<?php if ( $description ) : ?><p><?php echo esc_html( wp_trim_words( wp_strip_all_tags( $description ), 32 ) ); ?></p><?php endif; ?>
								<?php echo worldgraph_child_story_facts( array( __( 'Shot type', 'worldgraph-child' ) => $shot_type, __( 'Camera', 'worldgraph-child' ) => $camera, __( 'Lens', 'worldgraph-child' ) => $lens, __( 'Duration', 'worldgraph-child' ) => $duration ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Helper escapes fields. ?>
							</div>
						</li>
					<?php endwhile; ?>
				</ol>
			<?php else : ?>
				<?php echo worldgraph_child_story_empty_state( __( 'No published Shots yet', 'worldgraph-child' ), __( 'Publish Shots to build the storyboard.', 'worldgraph-child' ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Helper escapes fields. ?>
			<?php endif; ?>
		</section>
	</div>
	<?php
...
function worldgraph_child_render_project_story( $post, $payload, $detail ) {
	...
	<article class="wg-story-item wg-story-item--project <?php echo $detail ? 'is-detail' : 'is-card'; ?>">
		...
		<?php if ( $detail ) : ?>
			...
			?>
			<?php
			$storyboard_page = get_page_by_path( 'storyboard' );
			$storyboard_url  = $storyboard_page ? get_permalink( $storyboard_page ) : home_url( '/storyboard/' );
			?>
			<p class="wg-project-storyboard-link"><a class="wg-story-link" href="<?php echo esc_url( $storyboard_url ); ?>"><?php esc_html_e( 'Open storyboard', 'worldgraph-child' ); ?><span aria-hidden="true"> &rarr;</span></a></p>
