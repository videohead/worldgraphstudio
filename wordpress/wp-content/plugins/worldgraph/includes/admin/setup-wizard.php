		...
		<div class="wrap">
			...
			<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
				...
				<?php if ( 'local_mcp' === $comfy_mode ) : ?>
					...
					<p class="description">World Graph Studio's local fallback uses <strong><?php echo esc_html( \WorldGraph\Utils\Comfy_Bootstrap::DEFAULT_CHECKPOINT_LABEL ); ?></strong> (<code><?php echo esc_html( \WorldGraph\Utils\Comfy_Bootstrap::DEFAULT_CHECKPOINT ); ?></code>) with ComfyUI's stock nodes &mdash; no custom nodes required. <strong>You must install that checkpoint on the machine running ComfyUI</strong> unless another checkpoint compatible with the fallback workflow is already there: download <a href="<?php echo esc_url( \WorldGraph\Utils\Comfy_Bootstrap::DEFAULT_CHECKPOINT_URL ); ?>" rel="noopener noreferrer" target="_blank">the checkpoint file</a> into ComfyUI's <code>models/checkpoints</code> folder, then use ComfyUI's Refresh action (or restart ComfyUI) and re-check below. The checklist provisions the text-to-image <strong>Template</strong> World Graph Studio generates against and lists whatever is still missing.</p>
