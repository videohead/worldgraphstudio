...
class Setup_Wizard {
	...
	public static function render(): void {
		...
		<div class="wrap">
			...
			<?php endif; ?>
			<p>World Graph Studio requires a WordPress.org host or local Docker deployment. An API-connected LLM enables the World Graph Studio agents, while a generation Connection is optional for media generation.</p>
			...
			<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
				...
				<p>WordPress is connected. For production, configure a host scheduler to run WP-Cron. Local Docker users can run <code>docker compose exec wordpress wp cron event run --due-now</code>.</p>
				...
				<input type="url" class="regular-text" name="worldgraph_comfy_local_url" id="worldgraph_comfy_local_url" value="<?php echo esc_attr( get_option( 'worldgraph_comfy_local_url', 'http://host.docker.internal:8188' ) ); ?>" placeholder="http://host.docker.internal:8188" /> <span class="description">For ComfyUI running on the Docker host, use <code>http://host.docker.internal:8188</code>; <code>localhost</code> refers to the WordPress container. If ComfyUI shares this Compose network, use its service hostname instead.</span></p>
				...
				<p class="description">Direct connectors for services such as Sora, Runway, Veo, Kling, Firefly, Amazon video endpoints, and ByteDance-operated Seedance endpoints require additional API discovery and provider-specific implementation. Seedance 2.5 via CyberBara and MidJourney are configured separately on World Graph Studio &gt; Connections; the former uses a CyberBara API key, while the latter can use two independent service credentials.</p>
