...
= Midjourney-API.com REST =

Midjourney-API.com is a third-party bridge and is not represented here as an official Midjourney API. When an administrator tests this transport, the plugin sends authentication and a fixed non-generating task identifier to `https://api.midjourney-api.com`. When an authorized editor runs the REST Imagine Template, it sends the API key, prompt, reviewed speed mode, and timeout; it later sends the returned task identifier while polling. The service returns task state and image URLs, and the plugin downloads every final image into the WordPress Media Library.

A separate midjourney-api.com account, API key, quota, and any applicable usage payment are required. A Midjourney consumer subscription does not authenticate this intermediary. [Service documentation](https://docs.midjourney-api.com/api-reference/introduction), [Quickstart](https://docs.midjourney-api.com/quickstart), [Contact](https://www.midjourney-api.com/contacts).

= Ace Data Cloud MidJourney MCP =

Ace Data Cloud operates a separate third-party MidJourney MCP intermediary. When an administrator configures or tests this transport, submits an Imagine request, or checks a task, the plugin contacts `https://midjourney.mcp.acedata.cloud/mcp`. It sends Bearer authentication, MCP client/protocol metadata, reviewed tool names, prompts, speed/translation/split-image/timeout options, and task identifiers. It receives tool schemas, task state, and generated-image URLs, then the plugin downloads every final image into WordPress.

A separate Ace Data Cloud MidJourney service token and prepaid or usage-based package are required. Its token is not interchangeable with a midjourney-api.com key, an Ace Data Cloud platform-management token, or a Midjourney consumer subscription. [Service documentation](https://docs.acedata.cloud/en/mcp/midjourney), [Authentication](https://docs.acedata.cloud/en/authentication), [Terms](https://docs.acedata.cloud/en/resources/terms), [Privacy](https://docs.acedata.cloud/en/resources/privacy).

= Higgsfield REST API and hosted MCP =

When an administrator configures or tests Higgsfield, or an authorized editor uses one of the three reviewed Higgsfield Templates, the plugin contacts `https://platform.higgsfield.ai`. It sends the REST key ID/secret in an authorization header, prompts, allowlisted model and generation settings, supplied reference-media URLs, or authorized WordPress attachments through Higgsfield's presigned upload flow. Attachment bytes are PUT without the REST credential to the provider-returned presigned HTTPS storage host. The plugin later sends request IDs while polling and downloads returned image, video, or audio URLs into the WordPress Media Library. Higgsfield documents provider output availability for at least seven days; imported WordPress copies follow the site's own retention policy.

The separate hosted MCP service uses Higgsfield account OAuth, not the REST key pair. When an administrator chooses Connect, the plugin contacts Higgsfield's authorization, token, and dynamic-client-registration endpoints under `https://mcp.higgsfield.ai`, sends the World Graph Studio client name and site's exact admin callback URL during registration, requests `openid`, `email`, and `offline_access`, and stores the resulting access and refresh tokens encrypted in the Connection. Testing and catalog inspection send OAuth Bearer authentication plus MCP client/protocol metadata to `https://mcp.higgsfield.ai/mcp` and request `tools/list`. This integration does not send generation prompts through MCP or call discovered MCP tools.

A Higgsfield developer API credential is required for REST, and the MCP service uses an eligible Higgsfield account or subscription. API, subscription, credit, storage, or model charges may apply. The two credentials are not interchangeable. Higgsfield model schemas and availability may change, and some requests may be served by downstream model providers. [API documentation](https://docs.higgsfield.ai/docs), [MCP connection guide](https://higgsfield.ai/creator-hub/help-center/integrations/how-do-i-connect-higgsfield-to-ai-agent), [Terms](https://higgsfield.ai/terms-of-use-agreement), [Privacy](https://higgsfield.ai/privacy-policy).
